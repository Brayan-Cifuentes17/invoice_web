...
app.get("/products", async (req, res) => {
  try {
    connection.query(
      "SELECT P.NOMBRE, PL.* FROM PRODUCTO_LOTE PL JOIN PRODUCTO P ON PL.ID_PRODUCTO = P.ID_PRODUCTO;",
      (err, results) => {
        if (err) {
...
app.get("/employees", async (req, res) => {
  try {
    connection.query(
      'SELECT U.TIPO,P.ID_PERSONA, P.NOMBRES, P.APELLIDOS FROM USUARIO U JOIN PERSONA P ON U.id_PERSONA = P.id_persona WHERE U.TIPO="EMP";',
      (err, results) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.post("/createInvoices", async (req, res) => {
  ...
  try {
    ...
    connection.query(
      "INSERT INTO FACTURA (ID_FACTURA, FECHA, ID_CLIENTE, ID_VENDEDOR) VALUES (?, ?, ?, ?)",
      [invoiceNumber, date, id_customer, employee],
      (err, result) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
        ...
        items.forEach((item) => {
          ...
          connection.query(
            "INSERT INTO DETALLE_FACTURA (ID_FACTURA, ID_PRODUCTO, ID_LOTE, CANTIDAD, SUB_TOTAL, IVA, TOTAL_PAGAR, METODO_PAGO) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [
              invoiceNumber,
              item.id,
              item.lote,
              item.quantity,
              item.totalPrice,
              item.totalPrice * 0.19 + item.totalPrice,
              subtotal * 0.19 + subtotal,
              paymentMethod,
            ],
            (err) => {
              if (err) {
              ...
              connection.query(
                "UPDATE PRODUCTO_LOTE SET CANTIDAD = CANTIDAD - ? WHERE ID_PRODUCTO = ? AND ID_LOTE = ?",
                [item.quantity, item.id, item.lote],
                (err) => {
                  if (err) {
                    return res.status(500).json({ error: err.message });
                  }
...
app.put("/update/:id", async (req, res) => {
  ...
  try {
    ...
    connection.query(
      "UPDATE FACTURA SET ID_FACTURA = ?, FECHA = ? WHERE ID_FACTURA = ?",
      [invoiceNumber, date, invoiceId],
      (err) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
...
app.get("/lastInvoiceNumber", async (req, res) => {
  try {
    connection.query(
      "SELECT MAX(ID_FACTURA) AS lastInvoiceNumber FROM FACTURA;",
      (err, results) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
    ...
    res
      .status(500)
      .json({ error: "Error al obtener el último número de factura" });
  }
});
...
app.post("/createUser", async (req, res) => {
  ...
  try {
    // Verificar si id_persona ya existe en la tabla persona
    connection.query(
      ...
      (error, results) => {
        if (error) {
          ...
          return res
            .status(500)
            .json({ message: "Error al verificar el ID de persona" });
        }

        // Si ya existe, retornar un mensaje
        if (results.length > 0) {
          return res
            .status(400)
            .json({
              message:
                "El ID de persona ya existe, no se puede crear el usuario.",
            });
        }

        // Si no existe, proceder a insertar en la tabla persona
        connection.query(
          `INSERT INTO persona (ID_PERSONA, TIPO_DOCUMENTO, NOMBRES, APELLIDOS, CORREO, GENERO, DIRECCION, TELEFONO)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            id_persona,
            tipo_documento,
          ...
          (error) => {
            if (error) {
              ...
              return res
                .status(500)
                .json({ message: "Error al insertar en la tabla persona" });
            }

            // Insertar en la tabla usuario
            connection.query(
              `INSERT INTO usuario (LOGIN, CLAVE, TIPO, ID_PERSONA)
                            VALUES (?, ?, ?, ?)`,
              [login, clave, tipo, id_persona],
              (error) => {
                if (error) {
                  console.error(error);
                  return res
                    .status(500)
                    .json({ message: "Error al insertar en la tabla usuario" });
                }
